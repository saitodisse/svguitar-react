...
10. **Dado** que especifico um acorde com mais de 4 dedos pressionados (fret > 0) e `autoBarreEnabled` está ativo (padrão), **Quando** não há barres manuais definidas, **Então** o componente detecta automaticamente o traste com maior número de dedos e adiciona uma barre nesse traste, cobrindo todas as cordas do primeiro ao último dedo naquele traste. Os dedos cobertos pela barre são removidos da visualização, resultando em um diagrama limpo e profissional. Em caso de empate (múltiplos trastes com o mesmo número de dedos), a barre é posicionada no traste mais baixo.
...
- Como o auto barre interage com barres manuais? (Se houver qualquer barre manual definida via prop `barres`, o auto barre é desabilitado completamente, mesmo que `autoBarreEnabled` seja true. Barres manuais têm precedência total).
- O que acontece se `autoBarreEnabled` for false? (Nenhuma barre automática é adicionada, independentemente do número de dedos. O componente renderiza apenas os dedos e barres manuais especificados explicitamente).
- O que acontece se todos os dedos estiverem em trastes diferentes? (Auto barre não é acionado, pois nenhum traste individual tem mais de 1 dedo. O diagrama renderiza apenas os dedos individuais).
...
- **FR-025**: O sistema DEVE posicionar a numeração das posições dos trastes nas views verticais à direita do braço, exibindo valores crescentes de cima para baixo iniciando em "1" (sem exibir o nut). Ambas as views (`vertical-right` e `vertical-left`) DEVEM compartilhar o mesmo mapeamento de posições, sem inversão adicional na orientação vertical.
- **FR-026**: O sistema DEVE posicionar os números das posições dos trastes (FretNumbers) nas views verticais à direita do braço, nos pontos médios de cada casa do braço. O número "1" deve ser posicionado no ponto médio da casa 1 (entre o nut/traste 0 e o traste 1), o número "2" no ponto médio da casa 2 (entre o traste 1 e o traste 2), e assim por diante. O nut (traste 0) NÃO deve ter um número associado. Os números representam as casas onde os dedos são posicionados, formando uma coluna vertical ao lado direito do diagrama, mantendo legibilidade horizontal e consistência entre as views `vertical-right` e `vertical-left`. Os rótulos de afinação (TuningLabels) devem ser posicionados acima das cordas verticais, da esquerda para a direita.
...
- **FR-030**: O sistema DEVE permitir customização da largura/grossura das barres (pestanas) através da prop `barresWidth` (número em pixels, padrão: 8). Em views horizontais, controla a largura (width) do retângulo SVG; em views verticais, controla a altura/grossura (height) do retângulo SVG.
...
- **FR-034**: O sistema DEVE detectar automaticamente a necessidade de uma barre (pestana) quando houver mais de 4 dedos com fret > 0 (ignorando cordas soltas/mutadas), posicionando a barre no traste que possui o maior número de dedos. Em caso de empate (múltiplos trastes com o mesmo número de dedos), a barre DEVE ser posicionada no traste mais baixo (menor número). A barre automática DEVE cobrir da primeira à última corda com dedo naquele traste (fromString = menor string, toString = maior string). Os dedos individuais cobertos pela barre DEVEM ser removidos da renderização para evitar redundância visual.
- **FR-035**: O sistema DEVE permitir ativar/desativar a detecção automática de barres através da prop `autoBarreEnabled` (boolean, padrão: true) na interface pública `ChordDiagramProps`.
- **FR-036**: O sistema DEVE desabilitar a detecção automática de barres se houver qualquer barre definida manualmente via prop `barres` (barres manuais têm precedência sobre barres automáticas, garantindo controle total ao usuário quando necessário).
